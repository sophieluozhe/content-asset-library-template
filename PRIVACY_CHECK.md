...
- 模板库本地的 `00 内容控制台/双库同步原则.md` 属于内部管理规则，不得复制、暂存或上传到 GitHub。
- 私库中的同名内部管理规则、真实资料、真实看板条目和 `.obsidian` 配置同样不得复制、暂存或上传到 GitHub。
- 私库的真实资料、真实看板条目、`.obsidian` 配置和任何私库路径同样不得进入公开仓库。
